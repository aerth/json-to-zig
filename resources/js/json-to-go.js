/*
  ...
  JSON-to-Zig by aerth
	https://github.com/aerth/json-to-zig

	A simple utility to translate JSON into a Zig type definition.
*/
const xunknown = "*_unknown_type"; // TODO
...
function jsonToGo(json, typename, flatten = true, example = false, allOmitempty = false)
{
		...
		if (typeof scope === "object" && scope !== null)
		{
			if (Array.isArray(scope))
			{
				...
				if (sliceType == "struct") {
					...
					if (flatten && depth >= 2) {
						appender(sliceType || xunknown);
					} else {
						append(sliceType || xunknown);
					}
				}
			}
			else
	...
	function parseStruct(depth, innerTabs, scope, omitempty)
	{
		...
		if (flatten && depth >= 2)
		{
			...
			for (let i in keys)
			{
				...
				indenter(innerTabs)
				const typename = uniqueTypeName(format(keyname, true), seenTypeNames) 
	...
	function format(str, shouldCapitalize = true)
	{
		str = formatNumber(str);
  if (shouldCapitalize) {
    str = toProperCase(str);
  }
		let sanitized = str.replace(/[^a-z_0-9]/ig, "")
		if (!sanitized) {
